...
export default function JobSeekerProfile() {
  ...
  const fetchJobSeekerID = async () => {
    ...
    const uploadResume = async (event) => {
        ...
        console.log(file)
        try {
            const response = await axios.post(`${API_URL}/upload_resume`, formData, { headers:{"Authorization":`Bearer ${localStorage.getItem('authToken')}`,'Content-Type': 'multipart/form-data' } })
        ...
        const response = await axios.put(`${API_URL}/update_jobseeker/` + jobSeeker_id, { resumeURL: file.name }, { headers: {"Authorization":`Bearer ${localStorage.getItem('authToken')}`} }).then((result) => console.log(result))
